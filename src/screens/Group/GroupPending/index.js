...
const GroupPendingScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    API.getPendingGroupMemberByIDAPI(groupID).then((response) => {
  ...
  return (
    <View>
      <GoBackComponent bgColor={'#2fa'} textColor={'#333'} title={'Danh sách chờ duyệt'} />
      <View style={groupStype.container}>
        ...
        <Text style={groupStype.title}>Danh sách chờ duyệt</Text>
        <FlatList
        ...
        <Text style={groupStype.title}>Bài viết chờ duyệt</Text>
      </View>
    </View>
  )
}

export default GroupPendingScreen
