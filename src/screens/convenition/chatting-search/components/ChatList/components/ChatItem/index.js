...
const ChatItem = React.memo(
  ({ item, index, onLayout, historyTime, members, ownerID, beforeItem, onLongPress, conventionID, search }) => {
      ...
      return (
        <View>
          <Text style={{ textAlign: 'center', fontWeight: '400', fontSize: 13 }}>
            {sender?.aka ?? sender?.userName + item.message}
          </Text>
    ...
    if (!beforeItem) {
      ...
      const compareToCurrent = helper.DateTimeHelper.compareTwoDateByDate(Date.now(), historyTime, 'hour')
      const isMoreDay = Math.floor(compareToCurrent / 24) > 0
      const isMoreWeek = Math.floor(compareToCurrent / (24 * 7)) > 0
      const isMoreMonth = Math.floor(compareToCurrent / (24 * 7 * 30)) > 0
      let isTimeCompare = 'hour'

      if (isMoreDay) isTimeCompare = 'day'
       else if (isMoreWeek) isTimeCompare = 'week'
      else if (isMoreMonth) isTimeCompare = 'month'
      const timeSpace = helper.DateTimeHelper.compareTwoDateByDate(time, beforeTime, isTimeCompare)
     console.log(`time time of ${config.message} with type ${isTimeCompare} is ${timeSpace}`)
      return (
        <config.messageType
          _id={config._id}
          onLayout={onLayout}
          ...
          time={timeSpace >0 ? item.createdAt : undefined}
          avatar={config.avatar}
          owner={config.owner}
          onLongPress={onLongPress}
          detail={config.detail}
      ...
      const beforeTime = beforeItem.createdAt
      const compareToCurrent = helper.DateTimeHelper.compareTwoDateByDate(new Date(), historyTime, 'hour')
      const isMoreDay = Math.floor(compareToCurrent / 24) >0
      const isMoreWeek = Math.floor(compareToCurrent / (24 * 7)) > 0
      const isMoreMonth = Math.floor(compareToCurrent / (24 * 7 * 30)) > 0
      let isTimeCompare = 'hour'
      console.log({compareToCurrent})
